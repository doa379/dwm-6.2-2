...
enum { ClkTagBar, /* Customisation */ ClkKbLayout /* */, ClkStatusText, ClkWinTitle, ClkClientWin, ClkRootWin, ClkLast }; /* clicks */
...
struct Client {
  ...
  float mina, maxa;
  int x, y, w, h;
  int sfx, sfy, sfw, sfh; /* saved float geometry */
...
static void setlayout_float(const Arg *);
static void set_mfact(const Arg *);
...
static void setkblayout(const Arg *arg)
{
  (void) arg;
  selkb++;
  selkb %= LENGTH(kb_layouts);
  const char *cmd[] =  { "setxkbmap", kb_layouts[selkb], NULL };
  Arg a = { .v = cmd };
  spawn(&a);
  drawbar(selmon);
}
...
static void setlayout_float(const Arg *arg)
{
  setlayout(arg);
  setfloat();
}

static void inc_nmaster(const Arg *arg)
{
  settile();
  Arg a;
  a.i = arg->i;
...
static void viewnonempty(const Arg *arg)
{
...
static void floating(const Arg *arg)
{
